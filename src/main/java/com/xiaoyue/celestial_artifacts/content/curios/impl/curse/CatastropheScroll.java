package com.xiaoyue.celestial_artifacts.content.curios.impl.curse;
...
public class CatastropheScroll implements TickFacet, TextFacet, CAAttackToken {
	...
	public void onPlayerHurtTarget(Player player, AttackCache cache) {
		...
		if (CurioUtils.hasCurio(player, CAItems.ORIGIN_ETCHING.get())) {
			...
			damage = (float) (1 - CommonConfig.CATASTROPHE_SCROLL_ORIGIN_CURSE_DAMAGE.get());
		}
	...
	public void onPlayerDamaged(Player player, AttackCache cache) {
		...
		if (CurioUtils.hasCurio(player, CAItems.CHAOTIC_ETCHING.get())) {
			...
			if (getSource(cache).is(DamageTypes.EXPLOSION)) {
				factor = (float) (1 + CommonConfig.CATASTROPHE_SCROLL_EXPLOSION_DAMAGE.get());
			} else {
				factor = (float) (1 + CommonConfig.CATASTROPHE_SCROLL_OTHER_DAMAGE.get());
			}
		}
	...
	public void onPlayerDamagedFinal(Player player, AttackCache cache) {
		if (!CurioUtils.hasCurio(player, CAItems.END_ETCHING.get())) {
			if (cache.getDamageDealt() > player.getHealth() * CommonConfig.CATASTROPHE_SCROLL_END_CURSE_DAMAGE.get()) {
